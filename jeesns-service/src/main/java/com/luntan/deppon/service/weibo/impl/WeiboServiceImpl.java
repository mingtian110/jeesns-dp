...
import com.luntan.deppon.dao.weibo.IWeiboDao;
import com.luntan.deppon.service.weibo.IWeiboService;
import com.luntan.deppon.common.utils.ActionLogType;
...
//    @Transactional
    public ResponseModel save(HttpServletRequest request, Member loginMember, String content, String pictures) {
        if("0".equals(request.getServletContext().getAttribute(ConfigUtil.WEIBO_POST.toUpperCase()))){
            return new ResponseModel(-1,WeiboCommentServiceImpl.weiboAlias+"已关闭");
        }
...
//            scoreDetailService.scoreBonus(loginMember.getId(), ScoreRuleConsts.RELEASE_WEIBO, weibo.getId());
    ...
    @Override
    public ResponseModel<Weibo> listByFansPage(Page page, int memberId, int loginMemberId, String key) {
        if (StringUtils.isNotBlank(key)){
            key = "%"+key.trim()+"%";
        }
        List<Weibo> list = weiboDao.listByPage(page, memberId,loginMemberId,key);
...
//    @Transactional
    ...
    public ResponseModel delete(HttpServletRequest request, Member loginMember, int id) {
        ...
        if(weibo == null){
            return new ResponseModel(-1,WeiboCommentServiceImpl.weiboAlias+"不存在");
        }
        weiboDao.delete(id);
        //扣除积分
