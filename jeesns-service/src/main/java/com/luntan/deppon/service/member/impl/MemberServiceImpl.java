...
import com.luntan.deppon.common.utils.EmailSendUtil;
...
//            scoreDetailService.scoreBonus(findMember.getId(), ScoreRuleConsts.LOGIN);
    ...
    @Override
    public ResponseModel loginForCubc(Member member, HttpServletRequest request) {
        Map<String,String> config = configService.getConfigToMap();
        if("0".equals(config.get(ConfigUtil.MEMBER_LOGIN_OPEN))){
            return new ResponseModel(-1,"登录功能已关闭");
        }
        String password = member.getPassword();
        member.setPassword(Md5Util.getMD5Code(member.getPassword()));
        Member findMember = memberDao.loginForCubc(member);
        request.getServletContext().setAttribute(findMember.getName(),findMember);
        if(findMember != null){
            if(findMember.getStatus() == -1){
                return new ResponseModel(-1,"该账户已被禁用");
            }
...
//    @Transactional
    ...
    @Override
    public ResponseModel<Member> findSomeBody(Page page){
        List<Member> members = memberDao.listByPage(page, null);
        ResponseModel model = new ResponseModel(0, page);
        model.setData(members);
        return model;
    }
