...
public class GroupController extends BaseController {
    ...
    public String topicList(@RequestParam(value = "key",required = false,defaultValue = "") String key, Integer cateid,Model model) {
        Page page = new Page(request);
        if(cateid == null){
            cateid = 0;
        }
        ResponseModel groupTopicModel = groupTopicService.listByPage(page,key,cateid,1,0);
        model.addAttribute("groupTopicList",groupTopicModel);
        return jeesnsConfig.getFrontTemplate() + "/group/topicList";
    }
